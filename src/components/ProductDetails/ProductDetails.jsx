	...
	return (
		<>
			{isLoadin ? (
				...
				<div
					className=" fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center overflow-auto p-4"
					onClick={() => {
						dispatch(setShowModal(false));
						setSelectedImage(null);
					}}>
					<div
						className="bg-white rounded-lg w-full max-w-4xl min-h-[80vh] mt-10 overflow-y-auto p-3 md:p-6 relative"
						...
						<div className=" pt-7 w-full flex flex-col md:flex-row gap-6">
							...
							<div className="order-3 space-y-4 md:w-[40%]">
								...
								<div className="flex items-center gap-4 flex-wrap">
									...
									<button className=" flex-1 px-3 py-2 bg-green-600 text-white rounded hover:bg-green-700 transition">Add to Cart</button>
								</div>
								...
								<div className="mt-6">
									...
									<p className="text-sm text-gray-600 whitespace-pre-line">{product.description?.split(" ").slice(0, 10).join(" ") + "..."}</p>
								</div>
							</div>
						</div>
					</div>
