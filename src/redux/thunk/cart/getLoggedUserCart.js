...
export const getLoggedUserCart = createAsyncThunk("cart/getLoggedUserCart", async (_, { rejectWithValue }) => {
	try {
		const {
			data: { data },
		...
		if (axios.isAxiosError(error)) {
			...
			if (error.response) {
				...
				errorMessage = "There is a problem preventing you from see your products. Maybe you are not logged in yet?";
			}
