...
export const addProductToUserCart = createAsyncThunk("cart/addProductToUserCart", async (productId, { rejectWithValue }) => {
	try {
		const { data } = await axios.post("https://ecommerce.routemisr.com/api/v1/cart", productId.toString(), {
			headers: {
		...
		if (axios.isAxiosError(error)) {
			...
			if (error.response) {
				...
				errorMessage = "There is a problem preventing you from adding this product. Maybe you are not logged in yet?";
			}
